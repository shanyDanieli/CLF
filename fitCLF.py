...
plsq = leastsq(residuals_c, p0, args=(y, x_cen))
...
def peval_sat(x_lum,p): # still need to modify
    ...
    return phi_s*Lrat**(p[3]+1)*np.exp(-Lrat**2)

# Fitting the CLF for central galaxies
def residuals_sat(p,y,x_lum):
	M = 10**12.8
...
#	err = np.asarray([log(k,10) for k in y]) - np.asarray([log(p,10) for p in clf])
	err = y - clf
	#err = np.asarray([log(k+delta,10) for k in y]) - np.asarray([log(p+delta,10) for p in clf])
	return err
...
plsq_sat = leastsq(residuals_sat, p0_sat, args=(y, x_sat))
...
#y_fit_plot_sat = [log(n,10) for n in peval(x_fit_sat, plsq_sat[0])]
